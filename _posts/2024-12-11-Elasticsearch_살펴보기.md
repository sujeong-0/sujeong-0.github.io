...
: 클러스터가 여러 개면, 트래픽을 그만큼 나눠서 처리할 수 있기 때문에 부하 분산이 가능하다.
또한, 클러스터가 여러 개면, 하나의 클러스터가 죽어도 나머지 클러스터들이 처리할 수 있어 안정성이 높아진다.
...
>  
> 설정 내용 중에 `master: true` 와 같이 설정하는 값이 있는데, 이 값은 실제 마스터 노드를 지정하는 값이 아닌 **마스터 후보 노드를 지정하는 설정**이다.
...
: 마스터 노드는 검색 요청 처리를 안할 것 같지만, 마스터 노드도 처리한다.
...
**매핑**
: 문서의 구조를 나타내는 정보
: elasticsearch는 schemaless가 아니고 미리 정의하지 않아도 될 뿐 인것
